  ...
  const onFinish = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/auth/login", {
        method: "POST",
        body: JSON.stringify({ email, password }), // email ve password gönderiliyor
        ...
        <>
          ...
          <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center z-50">
            <div className="bg-white shadow-lg rounded-lg w-full max-w-md relative">
              ...
              {/* Giriş Formu */}
              <form className="px-6" onSubmit={onFinish}>
                <input
                  type="email"
                ...
                <button
                  type="submit"
                  className="ml-2 w-[381px] h-[45px] bg-cercevekirmizi text-white py-2 rounded hover:bg-red-600"
                >
                  Giriş Yap
                </button>
              </form>
